...
void SetStructOcenka(struct ocenka *st, char *string1, int integer1, int integer2, int integer3)
{
...
{   
    ...
    SetStructOcenka(&tmp_new->data.dannie_uspevaemosti[i], tmp3_predm, tmp3_ocenka1, tmp3_ocenka2, tmp3_ocenka3);
    }
...
void dobavlenie()
{
    ...
    SetStructOcenka(&tmp_new2->data.dannie_uspevaemosti[i], tmp_predm, tmp_ocenka1, tmp_ocenka2, tmp_ocenka3);
    }
   ...
   while (p!=NULL)
   {
...

        {
            if ((p->data.dannie_uspevaemosti[i].ocenka1 == findocenka)
                && (p->data.dannie_uspevaemosti[i].ocenka2 == findocenka)
                   && (p->data.dannie_uspevaemosti[i].ocenka3 == findocenka))
                    {

                        printf("\tФамилия: %s, Предмет: %s, Оценки: %d %d %d\n", p->data.surname, p->data.dannie_uspevaemosti[i].predm, p->data.dannie_uspevaemosti[i].ocenka1, p->data.dannie_uspevaemosti[i].ocenka2, p->data.dannie_uspevaemosti[i].ocenka3);
                        /*PrintStructZapis(&p->data);*/
                            printf("\n Удалить запись?");
                            fflush(stdout);
...
void LoadFile(FILE *tf) //НЕ РАБОТАЕТ
{
    char hr;
    system("clear");
